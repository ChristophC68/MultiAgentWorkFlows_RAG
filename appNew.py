...
from DynamoDB import dynamo_scripts
...
actual_session_id = ""
actual_session_id = random.randint(20010,99999999999999999)# The session id is a random value to just keep all questions asked by the same user related in DynamoDB!


# all prompts are now in ML Flow
...
def chunk_text(text, chunk_size=200, overlap=50):
    ...
    try:
        ...
        return chunks
            ...
            print(f"In chunk_text, Error Type: {type(e).__name__}")
...
def download_and_prepare():
    ...
    try:
        ...
        for filename, url in Global_documents.items():
            ...
            # if possible, try to recover from any one download error by skipping problem file but still building text files for the ones that work
            try:
                print(f"\ndownload file: {filename}...")
                # response = requests.get(url)
                response = requests.get(url, timeout=30)
                response.raise_for_status() # check if anything went wrong, this will raise an exception if so
                
                print(f"Length of downloaded pdf file content: {len(response.content)}") # hopefully we got this far
        ...
        return documents_to_chunk, True
    
    
    except Exception as e:
        print(f"In download_and_prepare, Error Type: {type(e).__name__}")
        print(f"Error Message: {e}")
        return [], False    
        ...
        #     print("\n" + "="*30 + " SUMMARISED RESPONSE " + "="*30 + "\n")
            ...
            try:
                ...
                if trace_id:
                    ...
                    try:
                        request_payload =  f"{full_request}" 
                        
                        if request_payload != "": 
                            # need to deal with true -> True
                            request_payload = request_payload.replace("true", "True")
                            # need to deal with null -> None   
                            request_payload = request_payload.replace("null", "None")
                            
                            request_payload = eval(request_payload) # convert the payload from str to dict
                            print(f"Use this request payload for debugging directly in imported file:\n{request_payload}\n")                  
                                                  
                            dynamo_scripts.insert_llm_record(
                        ...
                        response_payload = f"{full_response}"
                        if response_payload != "":
                            # need to deal with true -> True
                            response_payload = response_payload.replace("true", "True")
                            # need to deal with null -> None   
                            response_payload = response_payload.replace("null", "None")
                            
                            response_payload = eval(response_payload) # convert the payload from str to dict
                            print(f"Use this response payload for debugging directly in imported file:\n{response_payload}\n")                  
                            dynamo_scripts.insert_llm_record(
                                session_id=str(actual_session_id),
                                trace_id=str(trace_id),
                        ...
                        print(f"\n[ERROR in experimentRun, attempted insert of payloads into DynamoDB: {e}", file=sys.stderr)
                        return False, strAnswer
...
def main_download_prepare_chunk():
    try:  
        ...
        documents_to_chunk = []
        boolRes = False   
        documents_to_chunk, boolRes = download_and_prepare()
        ...
        if boolRes:
            ...
            print ("\nSkipping Chunking for this doc")
            return "Failure", []
...
def query_llm_worker(user_question, res_queue):
    ...
    try:
        ...
        try:
            ...
            connections.disconnect("default") # dont want to leave the Milvus connection hanging around
        except Exception:
            pass
        ...
        res_queue.put(f"In query_llm_worker, Error during query: {str(e)}")
